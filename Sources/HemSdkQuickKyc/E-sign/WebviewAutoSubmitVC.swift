...
///after ddpi sdk close automatically
//import UIKit
//import WebKit
//
//class WebviewAutoSubmitVC: UIViewController, WKNavigationDelegate {
...
//    }
//}
///congratulation msg after esign
import UIKit
import WebKit
...
extension WebviewAutoSubmitVC {
    ...
    func checkDDPIStatus() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
            if vc.ddpiSign == "1" {
                print("✅ DDPI Completed → Going back to ApplicationStatusVC")
                self.stopPolling()
                
                // ✅ Navigate back to ApplicationStatusVC instead of closing SDK
                self.navigationController?.popViewController(animated: true)
                
                // ✅ Show congratulation from parent VC
                vc.checkAndShowCongratulationIfNeeded()
            }
        }
    }
                ...
                case .success(let jsonResponse):
                    ...
                    if let errorCode = jsonResponse["ErrorCode"] as? String {
                        ...
                        case "000000":
                            ...
                            if isPdfSign == "1" {
                                print("🔥 DDPI SIGN COMPLETED → Navigate back to ApplicationStatusVC")
                                
                                DispatchQueue.main.async {
                                    self.stopPolling()
                                    
                                    // ✅ Navigate back to ApplicationStatusVC
                                    if let navController = self.navigationController {
                                        // Find ApplicationStatusVC in navigation stack
                                        if let appStatusVC = navController.viewControllers.first(where: { $0 is ApplicationStatusVC }) as? ApplicationStatusVC {
                                            // Pop to ApplicationStatusVC
                                            navController.popToViewController(appStatusVC, animated: true)
                                            
                                            // ✅ Update the UI and show congratulation
                                            appStatusVC.ViewAllMultiPDF()
                                            DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
                                                appStatusVC.checkAndShowCongratulationIfNeeded()
                                            }
                                        } else {
                                            // If not found in stack, just pop
