...
extension WebviewAutoSubmitVC {
    ...
    func checkDDPIStatus() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
            if vc.ddpiSign == "1" {
               ...
               // vc.closeSDK()   // 🔥 CLOSE FULL SDK
                self.showCongratulationsScreen()
            }
        }
    }
        ...
        CoreDataHelper.fetchAndRemoveFirstToken(entityName: "TokenMobile") { [weak self] tokenId in
            ...
            apiCall(url: url, method: "POST", parameters: parameters, view: self.view) { result in
                ...
                case .success(let json):
                                ...
                                if errorCode == "000000" {
                                    let isPDFSign = json["IsPDFSign"] as? String ??
                                                                           String(json["IsPDFSign"] as? Int ?? 0)
                                                            
                                                            if isPDFSign == "1" {
                                                                print("✅ DDPI SIGN COMPLETED → Show Congratulations")
                                                                
                                                                DispatchQueue.main.async {
                                                                    self.stopPolling()
                                                                    // Instead of directly closing, show congratulations
                                                                    self.showCongratulationsScreen()
                                                                }
                                                            } else {
                                                                print("⏳ DDPI not completed yet")
                                                            }
                                                        }
    ...
    func showCongratulationsScreen() {
           DispatchQueue.main.async {
               ...
               // Instantiate CongratulationsVC
               if let congratVC = storyboard.instantiateViewController(withIdentifier: "CongratulationVC") as? CongratulationVC {
                   
                   // Set the completion handler
                   congratVC.onOkTapped = { [weak self] in
                       print("✅ OK tapped on Congratulations screen")
                       
                       // Close the entire SDK
                       self?.closeSDKCompletely()
                   }
                   ...
                   // Fallback: If CongratulationsVC not found, just close SDK
                   print("⚠️ CongratulationsVC not found, closing SDK directly")
                   self.closeSDKCompletely()
               }
           }
       }
